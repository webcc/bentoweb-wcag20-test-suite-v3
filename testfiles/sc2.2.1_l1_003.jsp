...
<%! int count = 0; 
  String quote= ""; 
  String oldURL = "sc2.2.1_l1_003.jsp?timeout=false";
  String timeoutStr = "";
  String newURL = "";
  String newURLtext = "";
...
if ( request.getQueryString() != null) {
  timeoutStr = request.getParameter("timeout");
  if (timeoutStr.equalsIgnoreCase("true")) {
    timeout = true;
    oldURL = "sc2.2.1_l1_003.jsp?timeout=true";
    newURL = "sc2.2.1_l1_003.jsp?timeout=false";
    newURLtext = "Stop automatic update";
    badinput = false;
  } else if (timeoutStr.equalsIgnoreCase("false")) {
    timeout = false;
    oldURL = "sc2.2.1_l1_003.jsp?timeout=false";
    newURL = "sc2.2.1_l1_003.jsp?timeout=true";
    newURLtext = "Restart automatic update";
    badinput = false;
  } else {
    badinput = true;
    timeout = true;
...
<head>
    <title>Famous Quotes (BenToWeb Testfile)</title>
    <meta http-equiv="Content-Type" content="text/html;charset=utf-8" /><meta http-equiv="Pragma" content="no-cache" />
    <% if (timeout) { out.println("<meta http-equiv=\"refresh\" content=\"5\" />"); } %>
    <style type="text/css">
      q { display: block; }
    </style>
</head>
<body>
<% if (badinput) { out.println("<p><strong style=\"color: red; background: transparent;\">Suspicious input. Incident logged!</strong></p>"); } %><%-- Logging code should be added somewhere, but is not essential for this test suite. --%>
<p><a href="<%= newURL %>"><%= newURLtext %></a>.</p>
...
out.println(quote);
%>
<p><a href="<%= oldURL %>">Refresh this page</a> (and get a new quote).</p>
</body>
<!--
